...
const ChatContainer = () => {
  ...
  return (<>
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-700">
      <ChatHeader />
      {/* Messages */}
            ...
            return (
              <div key={message._id} className={`flex items-end ${isSender ? "justify-end" : "justify-start"}`}
                ...
                <div className={`w-10 h-10 rounded-full overflow-hidden border shrink-0 ${isSender ? "order-2 ml-3" : "order-1 mr-3"}`}>
                  <img src={isSender ? authUser?.avatar?.url || "/avatar-holder.avif" : selectedUser?.avatar?.url || "/avatar-holder.avif"} alt="/avatar-holder.avif" className="w-full h-full object-cover"/>
                </div>

                {/* BUBBLE */}
                <div className={`max-w-xs sm:max-w-sm md:max-w-md px-4 py-2 rounded-xl text-base 
                ${isSender ? "bg-slate-400 text-black order-1": "bg-slate-900 text-white order-2"}`}>
                  ...
                  <span className="block text-[10px] mt-1 text-right text-slate-100">
